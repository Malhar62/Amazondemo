...
export const OfferDetailScreen = observer(function OfferDetailScreen() {
  // Pull in one of our MST stores
  const { extraStore } = useStores()
  ...
  React.useEffect(() => {
    if (isFocused) {
      extraStore.getOfferListById(route.params.ID)
    }
  ...
  return (
    <View>
        ...
        <Button title='< prev'
          ...
          onPress={() => {
            if (route.params.ID > 1) {
              navigation.navigate('offerdetail', { ID: (route.params.ID - 1) })
            }
          }} />
        <Button title='next >'
          color='gold'
          onPress={() => {
            if (route.params.ID < (extraStore.offers.length - 1)) {
              navigation.navigate('offerdetail', { ID: (route.params.ID + 1) })
            }
          }} />
      ...
      <View>
        <Text style={{ fontSize: 20, alignSelf: 'center', borderBottomWidth: 1 }}>{route.params.ID}</Text>
        {extraStore.isLoading == true &&
          <View style={{ flexDirection: 'row', marginTop: 40, alignSelf: 'center' }}>
            <ActivityIndicator size='large' color='navy' />
            <Text style={{ fontSize: 20 }}>Loading...</Text>
          </View>}
        {extraStore.isLoading == false &&
          <FlatList
            data={extraStore.offerdetails}
            renderItem={({ item, index }) => (
              <View style={{ width: 200, height: 60, alignSelf: 'center', borderWidth: 1, marginTop: 20 }}>
                <Text style={{ fontSize: 20, color: 'black' }}>PRODUCT ID : {item.productId}</Text>
                <Text style={{ fontSize: 20 }}>QUANTITY : {item.quantity}</Text>
                <View>
                </View>
              </View>
            )}
